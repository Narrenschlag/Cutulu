...
namespace Cutulu
{
	public static class Physics
	{
		...
		public static bool Raycast(this Camera3D camera, out RaycastHit hit, float maxDistance)
		{
			Vector3 origin = camera.ProjectRayOrigin(camera.MousePosition());
			Vector3 dir = camera.ProjectRayNormal(camera.MousePosition());

			return Raycast(camera, origin, dir, out hit, maxDistance);
		}

		public static bool Raycast(this Node3D node, Vector3 globalOrigin, Vector3 direction, out RaycastHit hit, float maxDistance)
		{
			direction = direction.Normalized();

			var query = PhysicsRayQueryParameters3D.Create(globalOrigin, globalOrigin + direction * maxDistance);
